...
import { uploadDocuments, deleteDocument, getDocuments } from '../../controllers/documentController';
...
describe('Document Controller', () => {
  ...
  describe('uploadDocuments', () => {
    ...
    it('should upload a document successfully', async () => {
      ...
      // Call controller
      await uploadDocuments(mockRequest as Request, mockResponse as Response);
      ...
      expect(StudyService.generateStudyActivities).toHaveBeenCalledWith('test.pdf', userId);
      expect(statusSpy).toHaveBeenCalledWith(200);
      expect(jsonSpy).toHaveBeenCalledWith({
        data: { 
          docs: expect.objectContaining({
            documentId: expectedDocumentDTO.documentId,
            activityGenerationComplete: expectedDocumentDTO.activityGenerationComplete,
    ...
    it('should return 500 when an error occurs', async () => {
      ...
      Document.findOneAndUpdate.mockRejectedValue(new Error('Database error'));
  ...
  describe('deleteDocument', () => {
    ...
    it('should delete a document successfully', async () => {
      ...
      // Call controller
      await deleteDocument(mockRequest as Request, mockResponse as Response);
      
      // Assertions
      expect(Document.deleteOne).toHaveBeenCalledWith({ s3documentId: `${userId}-${documentId}` });
      expect(StudyService.deleteStudyActivities).toHaveBeenCalledWith(documentId, userId);
      expect(statusSpy).toHaveBeenCalledWith(200);
    ...
    it('should return 500 when an error occurs', async () => {
      ...
      (textractClient.send as jest.Mock).mockImplementationOnce(() => {
        return Promise.resolve({ 
          JobStatus: 'SUCCEEDED',
          Blocks: [
            { BlockType: 'LINE', Text: 'Extracted text from document' }
          ]
        });
      });
  ...
  describe('getDocuments', () => {
    ...
    it('should retrieve all documents for a user', async () => {
      ...
      // Call controller
      await getDocuments(mockRequest as Request, mockResponse as Response);
    ...
    it('should retrieve a specific document for a user', async () => {
      ...
      // Assertions
      expect(Document.findOne).toHaveBeenCalledWith({ documentId, userId });
      expect(statusSpy).toHaveBeenCalledWith(200);
      expect(jsonSpy).toHaveBeenCalledWith({
        data: { 
          docs: expect.objectContaining({
