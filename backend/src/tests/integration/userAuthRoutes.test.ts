...
jest.mock('../../services/userAuthService', () => {
  return {
    __esModule: true,
    default: {
      findCreateUser: jest.fn().mockImplementation(async (authPayload: AuthPayload) => {
...
describe('User Auth Controller', () => {
  ...
  describe('userAuthLogin', () => {
    it('should login a user successfully', async () => {
      ...
      // Call controller
      await userAuthLogin(mockRequest as Request, mockResponse as Response);

      // Assertions
      expect(userAuthService.findCreateUser).toHaveBeenCalledWith(authPayload);
